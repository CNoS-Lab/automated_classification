...
%         flip = input(['Flip ' curr_img '? (multiplies image by -1) y/n\n'],'s');
        if flip_val == 0
            flip = 'n';
        else
            flip = 'y';
        ...
        %% Version 2
        % Produce the average correlation between the incoming network and each 
        % exemplar using all significant slices
    %     fprintf('Calculating correlations over all slices (v2)...\n');
    % 
    %     [all_slices_C_pos, all_slices_C_neg, all_slices_A_pos, all_slices_A_neg] = open_slices(slices);
    % 
    %     ex_all_slices(size(exemplars,1)) = struct();
    %     for i = 1:size(ex_all_slices,2)
    %         j = shift(i);
    %         ex_all_slices(i).name = slices(j).N;
    %     end
    %     ex_all_slices = initialize_struct(ex_all_slices,size(exemplars,1),0);
    % 
    %     for num_ex = 1:size(exemplars,1)
    %         fprintf('Calculating correlations for exemplar %s/%s\n',num2str(num_ex),num2str(size(exemplars,1)));
    %         ex = exemplar_imgs(num_ex).img;
    %         if num_ex ~= 8 && num_ex ~= 9 && num_ex ~= 11
    %             [ex_slices,tm_slices] = get_all_slices(2,all_slices_C_pos,all_slices_A_pos,ex,to_match);
    %             ex_all_slices = perform_correlation(2,ex_all_slices,ex_slices,tm_slices,num_ex);
    %         else
    %             [ex_slices,tm_slices] = get_all_slices(2,all_slices_C_neg,all_slices_A_neg,ex,to_match);
    %             ex_all_slices = perform_correlation(2,ex_all_slices,ex_slices,tm_slices,num_ex);
    %         end
    %     end
    % 
    %     B = 2;
    % 
    %     v2_summary = write_summary(2,ex_all_slices);
    ...
    %             groups_ex(group).num_slices = size(C_slices,2);
    ...
    %             groups_ex(group).num_slices = size(A_slices,2);
    ...
    %     write_output(v1_summary,v2_summary,v3_summary,filepath,curr_img,comp_summary,num_exemplars,flip_ex);
...
%     fprintf(f,'Positive voxels (Sheet 1)\n\n');
    % for pos:
        order = order_groups('pos');
        % write excel rows for v3, make sure output order is correct
%         v3_name = vertcat(v1.name);
%         v3_pos = horzcat(v3(:).pos);
%         v3_pos_avg = sum(v3_pos(:,order),2)/size(v3_pos(order),2);
%         v3_pos_avg = [v3_pos_avg(1:7);v3_pos_avg(10:num_exemplars)];
%         v3_name = [v3_name(1:7);v3_name(10:num_exemplars)];
%         [v3_pos_avg_sorted,v3_pos_avg_idx] = sort(v3_pos_avg,'descend');
%         v3_name_sorted = v3_name(v3_pos_avg_idx);
%         fprintf(f,'Fisher Z based on pattern groupings (averaged) (v3)\n\n');
%         [v3_best_pos,max_ex] = max(v3_pos_avg);
%         fprintf(f,'Best match,%s\n',char(v3_name(max_ex)));
%         v3_best_match_pos = char(v3_name(max_ex));
%         for i = 1:size(vertcat(v1.name),1)
%             if i < 8
%                 fprintf(f,'%s,%.2f\n',char(v3_name_sorted(i)),v3_pos_avg_sorted(i));
%             elseif i > 9
%                 fprintf(f,'%s,%.2f\n',char(v3_name_sorted(i-2)),v3_pos_avg_sorted(i-2));
%             end
%         end
        % write excel rows for v1
        ...
        % write excel rows for v2
%         v2_pos = vertcat(v2.pos);
%         v2_name = vertcat(v2.name);
%         v2_pos = [v2_pos(1:7);v2_pos(10:num_exemplars)];
%         v2_name = [v2_name(1:7);v2_name(10:num_exemplars)];
%         [v2_pos_sorted,v2_pos_idx] = sort(v2_pos,'descend');
%         v2_name_sorted = v2_name(v2_pos_idx);
%         fprintf(f,'\nFisher Z ungrouped (v2)\n\n');
%         [v2_best_pos,max_ex] = max(v2_pos);
%         fprintf(f,'Best match,%s\n',char(v2_name(max_ex)));
%         v2_best_match_pos = char(v2_name(max_ex));
%         for i = 1:size(vertcat(v2.name),1)
%             if i < 8
%                 fprintf(f,'%s,%.2f\n',char(v2_name_sorted(i)),v2_pos_sorted(i));
%             elseif i > 9
%                 fprintf(f,'%s,%.2f\n',char(v2_name_sorted(i-2)),v2_pos_sorted(i-2));
%             end
%         end
    ...
    fprintf(f,'Component file: %s\n',[path,file]);
%     fprintf(f,'Negative voxels (Sheet 2)\n\n');
    % for neg:
        % write excel rows for v3, make sure output order is correct
        order = order_groups('neg');
%         v3_name = vertcat(v1.name);
%         v3_neg = horzcat(v3(:).neg);
%         v3_neg_avg = sum(v3_neg(:,order),2)/size(order,2);
%         v3_neg_avg = v3_neg_avg(8:9);
%         v3_name = v3_name(8:9);
%         [v3_neg_avg_sorted,v3_neg_avg_idx] = sort(v3_neg_avg,'descend');
%         v3_name_sorted = v3_name(v3_neg_avg_idx);
%         [v3_best_neg,max_ex] = max(v3_neg_avg);
%         v3_best_match_neg = char(v3_name(max_ex));
%         fprintf(f,'Fisher Z based on pattern groupings (averaged) (v3)\n\n');
%         fprintf(f,'Best match,%s\n',char(v3_name(max_ex)));
%         for i = 1:size(v3_neg_avg,1)
%                 fprintf(f,'%s,%.2f\n',char(v3_name_sorted(i)),v3_neg_avg_sorted(i));
%         end
%         v3_name = vertcat(v3.name);
        % write excel rows for v1
        ...
        % write excel rows for v2
%         v2_neg = vertcat(v2.neg);
%         v2_name = vertcat(v2.name);
%         v2_neg = v2_neg(8:9);
%         v2_name = v2_name(8:9);
%         [v2_neg_sorted,v2_neg_idx] = sort(v2_neg,'descend');
%         v2_name_sorted = v2_name(v2_neg_idx);
%         fprintf(f,'\nFisher Z ungrouped (v2)\n\n');
%         [v2_best_neg,max_ex] = max(v2_neg);
%         v2_best_match_neg = char(v2_name(max_ex));
%         fprintf(f,'Best match,%s\n',char(v2_name(max_ex)));
%         for i = 1:size(v2_neg,1)
%             fprintf(f,'%s,%.2f\n',char(v2_name_sorted(i)),v2_neg_sorted(i));
%         end
        % write all pattern matches
        fprintf(f,'\nFisher Z based on pattern groupings\n\n'); %(v3)
        ex_line(f,'neg');
...
%     merged = vertcat(v1_pos_sorted,v2_pos_sorted,v3_pos_avg_sorted,...
%                 v1_neg_sorted,v2_neg_sorted,v3_neg_avg_sorted);
    merged = vertcat(v1_pos_sorted,v1_neg_sorted);
    ymax = max(merged)+0.2;
    ymin = min(merged)-0.2;
    
%     Make the screen plots:
...
%     num_pos_ex = size(v1_pos,1);
    
%     subplot(1,3,1);
%     plot(1:num_pos_ex,v3_pos_avg_sorted,'Marker','o','MarkerFaceColor','b','MarkerSize',3);
%     title('Positives, v3');
%     ylim([ymin ymax]);
%     xlim([1,num_pos_ex]);
%     set(gca,'xtick',1:1:num_pos_ex,'ytick',0:0.5:2);

%     subplot(1,3,2);
%     plot(1:num_pos_ex,v1_pos_sorted,'Marker','o','MarkerFaceColor','b','MarkerSize',3);
%     title('Positives, v1');
%     ylim([ymin ymax]);
%     xlim([1,num_pos_ex]);
%     set(gca,'xtick',1:1:num_pos_ex,'ytick',0:0.5:2);
...
%     subplot(1,3,3);
%     plot(1:num_pos_ex,v2_pos_sorted,'Marker','o','MarkerFaceColor','b','MarkerSize',3);
%     title('Positives, v2');
%     ylim([ymin ymax]);
%     xlim([1,num_pos_ex]);
%     set(gca,'xtick',1:1:num_pos_ex,'ytick',0:0.5:2);
...
%     subplot(2,3,4);
%     plot(1:2,v3_neg_avg_sorted,'Marker','o','MarkerFaceColor','b','MarkerSize',3);
%     title('Negatives, v3');
%     ylim([ymin ymax]);
%     xlim([1,2]);
%     set(gca,'xtick',1:1:2,'ytick',0:0.5:2);
% 
%     subplot(2,3,5);
%     plot(1:2,v1_neg_sorted,'Marker','o','MarkerFaceColor','b','MarkerSize',3);
%     title('Negatives, v1');
%     ylim([ymin ymax]);
%     xlim([1,2]);
%     set(gca,'xtick',1:1:2,'ytick',0:0.5:2);
% 
%     subplot(2,3,6);
%     plot(1:2,v2_neg_sorted,'Marker','o','MarkerFaceColor','b','MarkerSize',3);
%     title('Negatives, v2');
%     ylim([ymin ymax]);
%     xlim([1,2]);
%     set(gca,'xtick',1:1:2,'ytick',0:0.5:2);
    ...
    fprintf('Classifying component: %s\n',file);
%     fprintf('\nResults for Positive loadings:\n');
    fprintf('\nNumber of positive voxels in component: %s (%s%%)\n', addComma(comp.numpos),perc_pos);
%     fprintf('Best match based on pattern grouping: %s, with Z = %s\n', v3_best_match_pos, num2str(v3_best_pos)); 
%     fprintf('Best match based on exemplar grouping: %s, with Z = %s\n', v1_best_match_pos, num2str(v1_best_pos)); 
%     fprintf('Best match based on no grouping: %s, with Z = %s\n', v2_best_match_pos, num2str(v2_best_pos)); 
    fprintf('Best match: %s, with Z = %s\n', v1_best_match_pos, num2str(v1_best_pos)); 
%     fprintf('\nResults for Negative loadings:\n');
    fprintf('\nNumber of negative voxels in component: %s (%s%%)\n', addComma(comp.numneg),perc_neg);
%     fprintf('Best match based on pattern grouping: %s, with Z = %s\n', v3_best_match_neg, num2str(v3_best_neg)); 
%     fprintf('Best match based on exemplar grouping: %s, with Z = %s\n', v1_best_match_neg, num2str(v1_best_neg)); 
%     fprintf('Best match based on no grouping: %s, with Z = %s\n', v2_best_match_neg, num2str(v2_best_neg)); 
    fprintf('Best match: %s, with Z = %s\n', v1_best_match_neg, num2str(v1_best_neg)); 

    fprintf('Done\n\n');
    
%     for i = 1:size(flip_ex,2)
%         if i ~= 8 && i ~= 9
%             if flip_ex(i) && abs(flip_val_pos(i)) > 0.6 % && abs(flip_val_pos(i)) > v1_best_pos
%                 da = sprintf('Deactivating %s found with Z = %s - check if component needs to be flipped, or notify Todd\n',char(v1(i).name),num2str(flip_val_pos(i)));
%                 fprintf(da);
%             end
%         else
%             if flip_ex(i) && abs(flip_val_neg(i)) > 0.6 % && abs(flip_val_neg(i)) > v1_best_neg
%                 da = sprintf('Activating %s found with Z = %s - check if component needs to be flipped, or notify Todd\n',char(v1(i).name),num2str(flip_val_neg(i)));
%                 fprintf(da);
%             end
%         end
%     end
